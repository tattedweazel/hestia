...
class SeriesVelocityJob(EtlJobV3):

	def __init__(self, target_date = None, db_connector = None, api_connector = None, file_location = ''):
		super().__init__(jobname = __name__, target_date = target_date, db_connector = db_connector, table_name = 'segment_series_velocity')
	...
	def load_data(self):
		...
		results = self.db_connector.read_redshift(f""" 
												WITH trial_views AS (
												    SELECT
												        ...
												        AVG(fv.active_seconds) as avg_time_watched,
												        COUNT(*) as total_views
												FROM
												    warehouse.vod_viewership fv
												INNER JOIN
												    warehouse.dim_segment_episode de
												ON
												    de.episode_key = fv.episode_key
												WHERE
												    fv.user_tier = 'trial' AND
												    fv.start_timestamp >= '{self.week_starting}' AND
												    fv.start_timestamp < '{self.query_date_cap}'
												GROUP BY 1
												),
												premium_views AS (
												    SELECT
												...
												ON
												    de.episode_key = fv.episode_key
												WHERE
												    fv.user_tier = 'free' AND
												    fv.user_key is not null AND
												    fv.start_timestamp >= '{self.week_starting}' AND
												    fv.start_timestamp < '{self.query_date_cap}'
												GROUP BY 1
												),
												anon_views AS (
												    SELECT de.series_title,
												           COUNT(distinct anonymous_id) as unique_viewers,
												           AVG(fv.active_seconds)       as avg_time_watched,
												           COUNT(*)                     as total_views
												    FROM 
												    ...
												    ON
												        de.episode_key = fv.episode_key
												    WHERE fv.user_tier = 'free'
												      AND fv.user_key is null
												      AND fv.start_timestamp >= '{self.week_starting}'
												      AND fv.start_timestamp < '{self.query_date_cap}'
												    GROUP BY 1
												),
												...
												SELECT
												    '{self.week_starting}' as week_starting,
												    '{self.week_ending}' as week_ending,
												    de.series_title,
												    de.channel_title,
												    ...
												    CASE WHEN pe.series_title IS NULL THEN 0 ELSE 1 END as posted_within_period,
												...
												LEFT JOIN
												    trial_views tv
												ON
												    tv.series_title = de.series_title
												LEFT JOIN
												    premium_views pv
												ON
												    pv.series_title = de.series_title
												LEFT JOIN
												    free_views fv
												ON
												    fv.series_title = de.series_title
												LEFT JOIN
												    anon_views av
												ON
												    av.series_title = de.series_title
												LEFT JOIN
												    published_episodes pe
												ON
												    pe.series_title = de.series_title
												WHERE
		...
		for result in results:
			record = {
				...
				"trial_viewers": result[4],
				"trial_views": result[5],
				"trial_avg_seconds_watched": result[6],
				"premium_viewers": result[7],
				"premium_views": result[8],
				"premium_avg_seconds_watched": result[9],
				"free_viewers": result[10],
				"free_views": result[11],
				"free_avg_seconds_watched": result[12],
				"anon_viewers": result[13],
				"anon_views": result[14],
				"anon_avg_seconds_watched": result[15],
				"total_views": result[16],
				"total_unique_viewers": result[17],
				"avg_seconds_watched": result[18],
				"posted_within_period": result[19],
				"most_recent_post": result[20]
			}
	...
	def execute(self):
		self.loggerv3.info(f"Running Series Velocity for {self.week_starting} - {self.week_ending}")
		self.load_data()
