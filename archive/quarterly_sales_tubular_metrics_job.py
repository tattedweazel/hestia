...
class QuarterlySalesTubularMetricsJob(EtlJobV3):

    def __init__(self, file_name, target_date=None, db_connector=None, api_connector=None, file_location=''):
        super().__init__(jobname=__name__, target_date=target_date, db_connector=db_connector, table_name='quarterly_sales_tubular_metrics', local_mode=True)
    ...
    def download_files_from_s3(self):
        ...
        key = self.prefix + self.file
        filename = '/'.join([self.downloads_directory, self.file])
    ...
    def execute(self):
        self.loggerv3.start(f"Running Quarterly Sales Tubular Metrics Job")
