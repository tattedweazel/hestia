...
class SponsoredPostsViewsAnalysis:
	...
	def __init__(self, authors, include_paid=True):
		...
		self.input_file = 'sponsored_input_views.csv'
		self.output_file = 'sponsored_posts_views.xlsx'
	...
	def read_files(self):
		...
		try:
			self.input_df = pd.read_csv(
				...
				usecols=['Channel', 'Author', 'Sponsor Name', 'Paid', 'Video Views', 'Paid Views'],
				encoding='utf-8',
				thousands=','
			)
		except:
	...
	def clean_dataframe(self):
		...
		for col in ['Video Views', 'Paid Views']:
		...
		if self.include_paid:
			# Subtract Paid Video Views
			self.input_df['Video Views'] = self.input_df['Video Views'] - self.input_df['Paid Views']

		# Only keep posts with Video Views
		self.input_df = self.input_df[self.input_df['Video Views'] != 0]

		# Identify Organic posts
		self.input_df['Sponsor Name'] = np.where(
			(self.input_df['Sponsor Name'].isnull()) & (self.input_df['Paid'].isnull()), 'Organic',
			self.input_df['Sponsor Name'])
	...
	def analyze_overall(self):
		...
		output = self.input_df.groupby(['Sponsor Name']).agg(
			avg_views=('Video Views', np.mean),
			posts=('Video Views', np.count_nonzero)
		)
		convert = {'avg_views': int}
