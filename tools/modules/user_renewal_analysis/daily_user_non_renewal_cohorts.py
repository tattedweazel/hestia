...
class DailyUserNonRenewalCohorts:
    ...
    def get_vod_viewership_by_batch(self, batch_ids):
        ...
        for result in results:
            vod_viewership.append({
                ...
                'series_watched': result[3],
                'episodes_watched': result[4]
            })
    ...
    def get_live_viewership_by_batch(self, batch_ids):
        ...
        results = self.db_connector.read_redshift(f"""
                                                     ...
                                                     WHERE 
                                                         ...
                                                         du.user_key in ({','.join(batch_ids)}) AND
                                                         lv.user_tier = 'premium'
                                                     GROUP BY 1, 2;
                                                 """
                                                   )
    ...
    def join_vod_live_viewership(self):
        ...
        viewership_df = pd.merge(vod_viewership_df, live_viewership_df, on=['user_key', 'viewership_date'], how='outer')
