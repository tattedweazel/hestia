...
class RenewalViewership:

    def __init__(self, start_month):
        """Identifies all of the users who did renewed in their X month by sub type and what they watched in that month"""
        self.start_month = start_month
        self.db_connector = DatabaseConnector('')
        self.loggerv3 = Loggerv3(name=__name__, file_location='', local_mode=1)
        self.table_name = 'renewal_viewership'
    ...
    def populate_user_viewership(self):
        ...
        for user_view in self.vod_viewership:
            ...
            last_renewal_date = max([renewal for renewal in self.users[user]['consecutive_renewals']])
            if signup <= user_view['viewership_date'] < last_renewal_date:
    ...
    def execute(self):
        self.loggerv3.start(f"Running Renewal Viewership for {self.start_month}")
