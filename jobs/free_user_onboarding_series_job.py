...
class FreeUserOnboardingSeriesJob(EtlJobV3):
    ...
    def __init__(self, target_date=None, db_connector=None, api_connector=None, file_location=''):
        ...
        self.output_filename = 'featured_series_free.json'
    ...
    def get_free_user_series(self):
        self.loggerv3.info('Getting free user series')
        query = f"""
            WITH last_n_days as (
                SELECT
                    user_key
    ...
    def get_svod_be_premium_series(self):
        self.loggerv3.info('Getting svod-be premium series')
        query = """
            SELECT uuid
            FROM shows
        ...
        for result in results:
            self.premium_series.append(result[0])
    ...
    def build_final_series(self):
        ...
        rank = 1
        self.final_series = []
        for series in self.free_series:
            if series['series_id'] not in self.premium_series:
                series['rank'] = rank
    ...
    def execute(self):
        self.loggerv3.start(f"Running Free User Onboarding Series Job for {self.target_date}")
        self.get_free_user_series()
        self.get_svod_be_premium_series()
