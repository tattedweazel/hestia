...
class WeeklyMedianPlatformViewershipJob(EtlJobV3):

	def __init__(self, target_date = None, db_connector = None, api_connector = None, file_location = ''):
		super().__init__(jobname = __name__, target_date = target_date, db_connector = db_connector, table_name = 'weekly_median_platform_viewership')
		...
		self.start_date = datetime.strftime(self.target_dt - timedelta(6), '%Y-%m-%d')
		self.start_dt = datetime.strptime(f"{self.start_date} 07:00", '%Y-%m-%d %H:%M')
	...
	def load_data(self):
		...
		for result in results:
			records.append({
				"week_starting": self.start_dt,
				"week_ending": self.target_dt,
				"platform": result[0],
	...
	def execute(self):
		self.loggerv3.info(f"Running Weekly Median Platform Viewership for {self.target_date}")
		self.load_data()
