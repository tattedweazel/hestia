...
class VoteShareCombinedJob(EtlJobV3):

	def __init__(self, target_date = None, db_connector = None, api_connector = None, file_location = ''):
		super().__init__(jobname = __name__, target_date = target_date, db_connector = db_connector, table_name = 'vote_share_combined')
	...
	def load_data(self):
		...
		query = f""" 
			WITH viewers as (
				...
				WHERE
				    ...
				    fsv.start_timestamp < '{self.query_end_cap}'
				GROUP BY 1,2,3
				ORDER BY 1,2,3
				), view_totals as (
				SELECT
				...
				WHERE
				    ...
				    fsv2.start_timestamp < '{self.query_end_cap}'
				GROUP BY 1
				), votes as (
				    SELECT v.user_key,
				           v.series_title,
		...
		for result in results:
			record = {
				...
				'vote_share_hours': result[2],
				'seconds_viewed': result[3],
				'vote_share_views': result[4],
				'total_views': result[5]			}
	...
	def execute(self):
		self.loggerv3.info(f"Running Vote Share Combined for {self.target_date}")
		self.load_data()
