...
class PremiumUserOnboardingEpisodesJob(EtlJobV3):
    ...
    def __init__(self, target_date=None, db_connector=None, api_connector=None, file_location=''):
        ...
        self.output_filename = 'featured_episodes_premium.json'
    ...
    def get_premium_user_episodes(self):
        self.loggerv3.info('Getting premium user episodes')
        query = f"""
            WITH user_viewership as (
                ...
                WHERE 
                    vv.start_timestamp > dateadd('days', {self.lookback_days}, '{self.target_date}')
                    AND vv.start_timestamp < dateadd('days', 1, '{self.target_date}')
                    AND vv.user_tier in ('trial', 'premium')
                ...
                SELECT
                    ...
                    completion_pct,
                    length_in_seconds,
                    round(completion_pct * unique_viewers, 0)                    as weight,
    ...
    def get_svod_be_live_episodes(self):
        ...
        uuids = [episode['episode_key'] for episode in self.premium_episodes]
    ...
    def build_final_episodes(self):
        ...
        rank = 1
        self.final_episodes = []
        for episode in self.premium_episodes:
            if episode['episode_key'] in self.live_episodes:
                episode['rank'] = rank
    ...
    def execute(self):
        self.loggerv3.start(f"Running Premium User Onboarding Episodes Job for {self.target_date}")
        self.get_premium_user_episodes()
