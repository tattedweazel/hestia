...
class DailyTubularMetricsJobV2(EtlJobV3):

    def __init__(self, target_date=None, db_connector=None, api_connector=None, file_location=''):
        super().__init__(jobname=__name__, target_date=target_date, db_connector=db_connector, table_name='daily_tubular_metrics_v2', local_mode=True)
        """This job also updates dim_tubular_videos table"""
        self.run_date = target_date
        self.file_location = file_location
        self.dim_table_name = 'dim_tubular_videos'
        ...
        self.files = [f"{self.run_date}.csv"]
    ...
    def read_records_from_files(self):
        ...
        for file in self.files:
            ...
            if self.records_df is None:
                self.records_df = pd.read_csv(filename, skiprows=2, low_memory=False)
            else:
    ...
    def write_to_dim_redshift_staging(self):
        ...
        self.db_connector.write_to_sql(self.dim_df, f'stage_{self.dim_table_name}', self.db_connector.sv2_engine(), schema=self.staging_schema, chunksize=5000, method='multi', index=False, if_exists='append')
        self.db_connector.update_redshift_table_permissions(f'stage_{self.dim_table_name}', self.staging_schema)
    ...
    def merge_dim_stage_to_prod(self):
        ...
        query = f"""
               BEGIN TRANSACTION;
                   ...
                   DELETE FROM {self.staging_schema}.stage_{self.dim_table_name}
                   USING {self.prod_schema}.{self.dim_table_name} prod
                   WHERE 
                       prod.video_id = stage_{self.dim_table_name}.video_id;

                   INSERT INTO {self.prod_schema}.{self.dim_table_name}
                   SELECT * FROM {self.staging_schema}.stage_{self.dim_table_name};

                   TRUNCATE {self.staging_schema}.stage_{self.dim_table_name};
    ...
    def build_final_dataframe(self):
        ...
        self.final_dataframe = self.final_dataframe[(self.final_dataframe['views'] > 0) & (self.final_dataframe['agg_type'] == 'daily')]
    ...
    def execute(self):
        self.loggerv3.start(f"Running Daily Tubular Metrics Job V2 for {self.run_date}")
