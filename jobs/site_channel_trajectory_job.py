...
class SiteChannelTrajectoryJob(EtlJobV3):

    def __init__(self, target_date = None, db_connector = None, api_connector = None, file_location = ''):
        super().__init__(jobname = __name__, target_date = target_date, db_connector = db_connector, table_name = 'site_channel_trajectory')
        ...
        query = f""" 
                WITH base_cte as (
                    ...
                    WHERE
                        ...
                        AND start_timestamp >= date_add('days', -121, '{self.target_date}')
                ), last_week as (
                    SELECT
                    channel_title,
                ...
                FROM last_week lw
                LEFT JOIN one_month_ago lm on lw.channel_title = lm.channel_title
                LEFT JOIN two_months_ago tma on lw.channel_title = tma.channel_title
                LEFT JOIN three_months_ago thma on lw.channel_title = thma.channel_title
                LEFT JOIN four_months_ago fma on lw.channel_title = fma.channel_title;
            """
    ...
    def execute(self):
        self.loggerv3.info(f"Running Site Channel Trajectory Job for {self.target_date}")
        self.load_data()
